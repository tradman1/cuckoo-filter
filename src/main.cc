...
int main(int argc, char *argv[]) {
  ...
  // string genome =
  // upload_genome("/home/jiva26/Documents/bioinf/GCF_000005845.2_ASM584v2_genomic.fna");
  ...
  outFile << "New test...uploaded genome from file: " << argv[1]
          << ", k = " << k << ", total_items = " << total_items
  ...
  outFile << "Inserted: " << 100.0 * num_inserted / total_items << "% items"
          << endl;
  ...
  outFile << "False positive rate = "
          << (float)false_queries / num_inserted * 100.0 << "%\n";
  const auto time = constr_time / static_cast<double>(1000 * 1000 * 1000);
  outFile << "Speed: " << (num_inserted / time) / (1000 * 1000)
          << " million keys/sec" << endl;
  // outFile << (float) false_queries / num_inserted * 100 << ";" <<
  // (num_inserted / time) / (1000 * 1000) << endl;
  outFile << "" << endl;
  return 0;
}